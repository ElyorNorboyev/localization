...
{{--    </div>--}}
    ...
    <a href="{{ route('users.create') }}" class="btn btn-info my-5">Create User</a>

    <h1 class="text-center text-uppercase">All Users</h1>
    <table class="table table-striped">
        ...
        @foreach($users as $user)
            <tr>
                ...
                <td><a href="{{ route('users.show',[$user]) }}">{{ Str::limit($user->name, 30) }}</a></td>
                <td>{{ Str::limit($user->email, 20) }}</td>
                <td>{{ Str::limit($user->password, 10)}}</td>
                <!-- <td>{{ $user->created_at }}</td> -->
                <!-- <td>{{ $user->updated_at }}</td> -->
                <td>
                    <a class="btn btn-link" href="{{ route('users.edit', [$user]) }}"><i
